...
bibliography: bibliographie_de_these
---


Tous les chunks jusqu'à "merge03" ne sont pas évalués (eval=F) puisque ceux-ci sont désormais inutiles après la création du fichier de données en format RData qui est chargé ci-dessous dans le chunk "charger_Pok2001.2022.RData".
...
```{r Fonctions, eval=F, include=F, message=F, warning=F}
# Pour lister les noms de variables (en-têtes des colonnes):
names(Pok2001.2022)

# Pour voir les premières lignes d'un tableau
head(Pok2001.2022)
# Pour voir les cinq premières lignes et les colonnes 5 à 10:
Pok2001.2022[1:5, 5:10]

# Pour savoir de quoi est constituée la variable
# Rechercher les variables comportant le mot "date":
names(Pok2001.2022[grep("campagne",names(Pok2001.2022), ignore.case=T)])

# On trouve trois variables avec le terme "date":
...
```{r Aggregate,  eval=F, include=FALSE}
...
```{r Relations_ordre_caract, eval=F}
# names(bv1)
...
Le suivi de la qualité de l'eau de la rivière Pokemouche a été effectué ~~en deux temps, en 2001-2002 et en 2021-2022 soit 20 ans après~~ **à douze stations situées pour la plupart à l'embouchure des tributaires de la branche principale. Le choix des stations a été déterminé par l'équipe du Cgerp lors de sa fondation en 2001, et le premier rapport du groupe ne donne pas de détails sur les raisons qui ont motivé l'emplacement des stations [@cgerp_comite_2003]**. Nos données ont été collectées à partir de 12 stations de surveillance réparties tout au long de la rivière, de l’amont à l’aval suivant le sens de l’écoulement de la rivières (de l’est à l’ouest), avec des prélèvements effectués 4 à 5 fois par an pour observer le dessin de la variation saisonnière (été vs automne) et celle observé en 20 ans plus tard.

AP 2024-06-19 Parler à Jean-Luc pour en savoir plus sur ce qui a guidé la sélection des stations.

![](./Pokemouche_12_stations.jpg)

## Le bassin dans son ensemble

Un bassin hydrographique, encore appelé bassin versant, est une surface géographique délimitée par des courants et des lignes d’eau, se déversant les unes dans les autres, constituée d’un point plus haut qui est l’amont et d’un point le plus bas qui est l’aval ( **référence** ). Au Nouveau-Brunswick, le règlement sur la classification des eaux ( **référence** ) se regroupe en eaux souterraines et eaux de surface. Nous distinguons les cours d’eau comme les rivières et leurs tributaires ; les lacs, les étangs, les réservoirs et les eaux des milieux humides. Nous pouvons y ajouter aussi des eaux estuariennes qui sont des zones stratégiques où une rivière rencontre et dilue l’eau marine, et l’eau dans les terres humides associée à ces estuaires (GNB, Classification des eaux du N-B, 2002).

Le bassin hydrographique de la rivière Pokemouche **couvre environ 504 km^2^ et** peut être divisé en 12 sous-bassins versants. ~~Le bassin hydrographique de la péninsule acadienne couvre environ 504 kilomètres carrés, avec une superficie de 44,316 hectares.~~ 75% de ces terres sont des terres privées et 25% représentent l’ensemble des terres de l’État ( \_référence\_\_ ). Le bassin hydrographique de la rivière Pokemouche abrite à peu près 13,000 personnes composées de deux villages, Paquetville et St-Isidore, dans une zone profondément rurale. Dans le bassin, les populations sont des pêcheurs, des travailleurs dans les tourbières, des agriculteurs et des propriétaires de terres boisées [@cgerp_comite_2003].
...
**AP 2024-06-19 Expliqure au lecteur l'ordre dans lequel tu présentes les stations. Toujours suivre le même ordre pour ne pas fatiguer le lecteur.**
...
AP 2024-06-19 Dans la description des variations, toujours essayer de suivre le même patron: 1) un énoncé ou deux sur la variabilité entre stations et entre périodes (box-plot); 2) un énoncé sur la variabilité saisonnière (graphique des campagnes); 3) un énconé sur les recommandations canadiennes; 4) un énoncé sur les valeurs obervées comparées au recommandations canadiennes.
...
On remarque que la variation périodique de 2021 à 2022 est comprise entre 7mg/L et plus de 13mg/L. On constat que malgré la variation saisonnière de hausse et de baisse qui se remarque sur les stations entre les deux année a augmenter. Sur cette période on remarque bien l'allure de la station 01 qui à connu de forte baisse et hausse avec la plus faible teneur de la période et la station 02 avec la plis forte teneur durant la période.
...
ggplot(aes(y=pH_units, x=Station2, fill=Period), data=Pok2001.2022) + geom_boxplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        ...
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  du pH sur chaque station à 20ans entre les deux Period")
```

Commentaire:

Le pH dans la période 1 est plus élevé que le pH de la période 2 et cela sur tout les stations.

Le pH de la stations 04, 01, 09, 11 et 07 indique une plus grande variabilité sur la période 2021-2022 par rapport au pH en 2001-2002.

Le pH présentes à deux stations de valeur abérante uniquement sur la periode de 2021-2022.
...
ggplot(aes(y=Turbidity_NTU, x=Period), data=Pok2001.2022) + geom_boxplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        ...
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation à 20ans entre les deux Période")
summary(Pok2001.2022$Turbidity_NTU)
```

#### Supprimer quelque points, pour voir mieux la variation à 20ans

Supprimer points déviants en créant un subset

```{r Turbidi, echo=F,message=F, warning=F}
# dim(Pok2001.2022)
Turbidi=subset(Pok2001.2022, Turbidity_NTU<15)
# Les points au dessus de 1000 ont été exclus
# Reprendre le même code mais changer "data=Pok2001.2022" pour "data=turbLT10"
Turbidi_ggp=ggplot(aes(y=Turbidity_NTU, x=Period), data=Turbidi) + geom_boxplot() +
...
# boxplot(Pok2001.2022$E.coli_MPN_100.mL ~  Pok2001.2022$Station)
...
#### Supprimer points, pour voir mieux la variation à 20ans

Supprimer points déviants en créant un subset

```{r E_colia, echo=F,message=F, warning=F}
# dim(Pok2001.2022)
E_colia=subset(Pok2001.2022, E.coli_MPN_100.mL<1000)
# Les points au dessus de 1000 ont été exclus
# Reprendre le même code mais changer "data=Pok2001.2022" pour "data=turbLT10"
E_colia_ggp=ggplot(aes(y=E.coli_MPN_100.mL, x=Period), data=E_colia) + geom_boxplot() +
...
# boxplot(Pok2001.2022$Nitrate_as_N_mg_L ~  Pok2001.2022$Station)
...
ggplot(aes(y=Nitrate_as_N_mg_L, x=Period), data=Pok2001.2022) + geom_boxplot() +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        ...
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation à 20ans entre les deux Periodes")
...
# ce 22 08 2024 à 16:33 j'ai collé les manquant du script et c'est a partie d'ici

#Analyse du deuxième objectif
...
# AP 2024-08-26 Attention à créer les "variables universelles", comme indiqué
# dans mon courriel du 2024-07-17 avec la fonction "ifelse".
# Ces nouvelles variables territoriales présentent les valeurs de 1990
# pour les rangées de 1990, et les valeurs de 2020 pour les rangées de 2020.
...
#Ce vendredi 12 juillet 2024 les variables en %

# Superficie non forestière et paramètre
...
# Ce lundi 15 juillet 2024

Je refaire le même schéma d'analyse juste avec les données de pourcentage
...
#essaie du code que Alain ma envoyé par courriel le lundi 15 à 15h46

```{r Agriculture_vs_Temperature }

# Reçu par courriel de Gaël 2024-07-15 lundi au campus pendant les vacances

# Créer les variables par période:
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,Temp_eau_YSI_degC))

subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,Temp_eau_YSI_degC))


# Attention à prendre le tableau "Period1" pour les graphiques de 1990

# plutôt que le tableau "Pok2001.2022".


ggplot() +

# Placer d'abord les points de la série "Period1":

geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Temp_eau_YSI_degC,

 color="P1", shape="P1")) +

geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Temp_eau_YSI_degC),

 method=lm, se=FALSE, color="red") +

# Ajouter les points pour la série "Period2"

 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Temp_eau_YSI_degC),

 method=lm, se=FALSE, color="blue") +

# Contrôler la couleur des séries
...
# Contrôler les symboles des séries
...
# Ajuster les titres
 ...
 labs(title="Relation entre Température et l'agriculture en 1990",
...
# non_foret_vs_température

```{r non_foret_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et la variable non forêt en 1990",
x="Non forêt (fraction de la superficie du bassin)",
y="Temp_eau_YSI_degC") +
...
# non_foret_vs_Oxygène

```{r non_foret_vs_Oxygène }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la variable non foret et l'Oxygène dissous en 1990",
...
labs(title="Relation entre la conductivité et la variable non foret ",
x="non foret (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le pH et la variable non_foret en 1990",
x="Non_foret (fraction de la superficie du bassin)",
y="pH_units") +
...
# non_foret_vs_turbidité

```{r non_foret_vs_turbidité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=log10(Turbidity_NTU),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=log10(Turbidity_NTU)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=log10(Turbidity_NTU),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=log10(Turbidity_NTU)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et la variable non_foret",
x="Non_foret (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# non_foret_vs_Couleur

```{r non_foret_vs_couleur }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et la variable non_foret",
x="Non_foret (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les COT et la variable non_foret",
x="Non_foret (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# non_foret_vs_coliforme

```{r non_foret_vs_coliforme }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,E.coli_MPN_100.mL))

# AP 2024-08-26 chunk 191; je reformate le code; les parenthèses n'étaient
# pas bien placées.
ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="red") +
geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=log10(E.coli_MPN_100.mL),
 color="P2", shape="P2")) +
geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
scale_color_manual(name="",values=c("P1"="red", "P2"="blue"), breaks=c("P1","P2")) +
scale_shape_manual(name="",values=c("P1"=1, "P2"=2), breaks=c("P1","P2")) +
labs(title="Relation entre les coliformes et la variable non_foret",
 x="Non_foret (fraction de la superficie du bassin)",
 y="E.coli_MPN_100.mL") +
theme_bw()
...
# non_foret_vs_phosphore

```{r non_foret_vs_phosphore }

#names(Pok2001.2022) 
# AP 2024-08-26: ce code (chunk 192) est OK
subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Phosphorus_Total_mg_L))

subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et la variable non_foret",
x="Non_foret (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# non_foret_vs_nitrate

```{r non_foret_vs_nitrate }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctNF_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctNF_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctNF_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctNF_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctNF_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctNF_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et la variable non_foret",
x="Non_foret (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Agriculture_vs_température

```{r Agriculture_vs_température }
...
labs(title="Relation entre la température et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Temp_eau_YSI_degC") +
...
# Agriculture_vs_Oxygène

```{r Agriculture_vs_Oxygène }
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
labs(title="Relation entre la conductivité et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre le pH et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="pH_units") +
...
labs(title="Relation entre la turbidité et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Agriculture_vs_couleur

```{r Agriculture_vs_Couleurs }
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les carbones organique totale et l'agriculture",
x="Agriculture (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Agriculture_vs_Coliforme

```{r Agriculture_vs_Coliforme}
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=E.coli_MPN_100.mL),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=E.coli_MPN_100.mL,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=E.coli_MPN_100.mL),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
#Agriculture_vs_Phosphore

```{r Agriculture_vs_Phosphore}
...
subset(Period1, select=c(Station2, Date, pctAGR_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctAGR_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctAGR_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctAGR_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et l'agriculture en 1990",
x="Agriculture (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Zone_industriel_vs_température

```{r Zone_industriel_vs_température}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Temp_eau_YSI_degC") +
...
# Zone_industriel_vs_Oxyg_dissous

```{r Zone_industriel_vs_Oxyg_dissous }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène dissous et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# Zone_industriel_vs_Conductivité

```{r Zone_industriel_vs_Conductivité_4}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Conductivity_microS_cm))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Conductivity_microS_cm))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Conductivity_microS_cm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Conductivity_microS_cm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la conductivité et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
# Zone_industriel_vs_pH

```{r Zone_industriel_vs_pH}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le potentiel d'hydrogène et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="pH_units") +
...
# Zone_industriel_vs_turbidité

```{r Zone_industriel_vs_turbidité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Zone_industriel_vs_couleur

```{r Zone_industriel_vs_couleur }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
# Zone_industriel_vs_COT

```{r Zone_industriel_vs_COT }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le carbone organique totale et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Zone_industriel_vs_coliforme

```{r Zone_industriel_vs_coliforme }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Zone_industriel_vs_phosphore

```{r Zone_industriel_vs_phosphore }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Zone_industriel_vs_nitrate

```{r Zone_industriel_vs_nitrate }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctIND_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctIND_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctIND_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctIND_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctIND_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctIND_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et la Zone industriel",
x=" Zone industriel (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
```{r Infrastructures_vs_température }
...
labs(title="Relation entre la température et les infrastructures",
...
# Infrastructures_vs_Oxyg_dissous

```{r Infrastructures_vs_Oxyg_dissous }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène dissous et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# Infrastructures_vs_Conductivité

```{r Infrastructures_vs_Conductivité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,Conductivity_microS_cm))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,Conductivity_microS_cm))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=Conductivity_microS_cm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=Conductivity_microS_cm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la conductivité et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
# Infrastructures_vs_pH

```{r Infrastructures_vs_pH }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le pH et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="pH_units") +
...
# Infrastructures_vs_turbidité

```{r Infrastructures_vs_turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=log10(Turbidity_NTU),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=log10(Turbidity_NTU)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=log10(Turbidity_NTU),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=log10(Turbidity_NTU)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
labs(title="Relation entre la couleur et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
labs(title="Relation entre la COT et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Infrastructures_vs_Coliforme

```{r Infrastructures_vs_Coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Infrastructures_vs_phosphore

```{r Infrastructures_vs_phophore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Infrastructures_vs_nitrate

```{r Infrastructures_vs_nitrate}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctINF_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctINF_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctINF_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctINF_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctINF_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctINF_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et les infrastructures",
x=" Les infrastructures (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Zone_urbaine_vs_température

```{r Zone_urbaine_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Zones urbaines",
...
# Zone_urbaine_vs_oxygène dissous

```{r Zone_urbaine_vs_oxygène_dissous }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène dissous et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# Zone_urbaine_vs_Conductivité

```{r Zone_urbaine_vs_Conductivité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,Conductivity_microS_cm))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,Conductivity_microS_cm))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=Conductivity_microS_cm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=Conductivity_microS_cm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la conductivité et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
```{r Zone_urbaine_vs_pH }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le potentiel d'hydrogène et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="pH_units") +
...
labs(title="Relation entre la turbidité et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Zone_urbaine_vs_Couleur

```{r Zone_urbaine_vs_Couleur }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
```{r Zone_urbaine_vs_COT }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le COT et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Zone_urbaine_vs_coliforme

```{r Zone_urbaine_vs_coliforme }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSET_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctSET_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctSET_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSET_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctSET_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctSET_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
labs(title="Relation entre le phosphore et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
labs(title="Relation entre le nitrate et les Zones urbaines",
x=" Les Zones urbaines (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Zone_recreative_vs_température

```{r Zone_recreative_vs_température }

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
#geom_point(data=Period1, aes(x=pctREC_supdrainee_1990, y=Temp_eau_YSI_degC,
# color="P1", shape="P1")) +
#geom_smooth(data=Period1, aes(x=pctREC_supdrainee_1990, y=Temp_eau_YSI_degC),
            
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Zones récréatives",
...
labs(title="Relation entre l'oxygène et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# Zone_recreative_vs_conductivité

```{r Zone_recreative_vs_conductivité}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Conductivity_microS_cm))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la conductivité et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
# Zone_recreative_vs_pH

```{r Zone_recreative_vs_pH}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,pH_units))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le pH et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="pH_units") +
...
# Zone_recreative_vs_Turbidity

```{r Zone_recreative_vs_Turbidity}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Turbidity_NTU))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Zone_recreative_vs_Colour

```{r Zone_recreative_vs_Colour}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Colour_TCU))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
# Zone_recreative_vs_COT

```{r Zone_recreative_vs_COT}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Carbon_Total_Organic_mg_L))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le COT et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Zone_recreative_vs_coliforme

```{r Zone_recreative_vs_coliforme}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,E.coli_MPN_100.mL))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Zone_recreative_vs_Phosphore

```{r Zone_recreative_vs_Phosphore}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Phosphorus_Total_mg_L))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Zone_recreative_vs_nitrate

```{r Zone_recreative_vs_nitrate}

#names(Pok2001.2022) 

#subset(Period1, select=c(Station2, Date, pctREC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctREC_supdrainee_2020,Nitrate_as_N_mg_L))
...
# method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctREC_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctREC_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et les Zones récréatives",
x=" Les Zones récréatives (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Zone_forets_vs_température

```{r Zone_forets_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Zones forêts",
...
# Zone_forets_vs_oxygène

```{r Zone_forets_vs_Oxygène }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'Oxygène dissous et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
labs(title="Relation entre la conductivité et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le pH et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="pH_units") +
...
# Zone_forets_vs_turbidité

```{r Zone_forets_vs_turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=log10(Turbidity_NTU),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=log10(Turbidity_NTU)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=log10(Turbidity_NTU),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=log10(Turbidity_NTU)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Zone_forets_vs_Couleur

```{r Zone_forets_vs_Couleur}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
```{r Zone_forets_vs_COT}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la COT et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Zone_forets_vs_Coliforme

```{r Zone_forets_vs_Coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Zone_forets_vs_phosphore

```{r Zone_forets_vs_phosphore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Zone_forets_vs_nitrate

```{r Zone_forets_vs_nitrate}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctFO_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctFO_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctFO_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctFO_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctFO_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctFO_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et les Zones forêts",
x=" Les Zones forêts (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Terres_humides_vs_température

```{r Terres_humides_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Terres humides",
...
# Terres_humides_vs_oxygène

```{r Terres_humides_vs_oxygène }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'Oxygène et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
```{r Terres_humides_vs_conductivité}
...
labs(title="Relation entre la conductivité et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre le pH et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="pH_units") +
...
# Terres_humides_vs_turbidité

```{r Terres_humides_vs_turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Terres_humides_vs_couleur

```{r Terres_humides_vs_couleur}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
# Terres_humides_vs_COT

```{r Terres_humides_vs_COT}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la COT et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Terres_humides_vs_coliforme

```{r Terres_humides_vs_coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=log10(E.coli_MPN_100.mL),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=log10(E.coli_MPN_100.mL)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=log10(E.coli_MPN_100.mL),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=log10(E.coli_MPN_100.mL)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le coliforme et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Terres_humides_vs_phosphore

```{r Terres_humides_vs_phosphore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
# Terres_humides_vs_nitrate

```{r Terres_humides_vs_nitrate}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctWL_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctWL_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctWL_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctWL_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctWL_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctWL_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et les Terres humides",
x=" Les Terres humides (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Somme_ordre_vs_température

```{r Somme_ordre_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Temp_eau_YSI_degC),
...
labs(title="Relation entre la température et la sommes des ordres",
...
# Somme_ordre_vs_Oxyg_dissous

```{r Somme_ordre_vs_Oxyg_dissous}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Oxyg_dissous_YSI_mg_L),
...
labs(title="Relation entre l'Oxyg_dissous et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
labs(title="Relation entre la Conductivité et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre le pH et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="pH_units") +
...
# Somme_ordre_vs_Turbidité

```{r Somme_ordre_vs_Turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Turbidity_NTU),
...
labs(title="Relation entre la Turbidité et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Turbidity_NTU") +
...
# Somme_ordre_vs_couleur

```{r Somme_ordre_vs_couleur}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Colour_TCU),
...
labs(title="Relation entre la couleur et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Colour_TCU") +
...
```{r Somme_ordre_vs_COT}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Carbon_Total_Organic_mg_L),
...
labs(title="Relation entre la COT et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Somme_ordre_vs_Coliforme

```{r Somme_ordre_vs_Coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=E.coli_MPN_100.mL),
...
labs(title="Relation entre les coliformes et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="E.coli_MPN_100.mL") +
...
# Somme_ordre_vs_phosphore

```{r Somme_ordre_vs_phosphore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Phosphorus_Total_mg_L),
...
labs(title="Relation entre le phosphore et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Phosphorus_Total_mg_L") +
...
# Somme_ordre_vs_Nitrate_as_N_mg_L

```{r Somme_ordre_vs_Nitrate_as_N_mg_L}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctSomme_ordre_1.5,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctSomme_ordre_1.5, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctSomme_ordre_1.5, y=Nitrate_as_N_mg_L),
...
labs(title="Relation entre le nitrate et la sommes des ordres",
x=" Sommes des ordres (Ordre des ruisseaux du sous bassin)",
y="Nitrate_as_N_mg_L") +
...
# superficie_champs_vs_température

```{r superficie_champs_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les superficies des champs",
...
# superficie_champs_vs_Oxygène

```{r superficie_champs_vs_Oxygène }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# superficie_champs_vs_Conductivité

```{r superficie_champs_vs_Conductivité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Conductivity_microS_cm))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Conductivity_microS_cm))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=Conductivity_microS_cm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=Conductivity_microS_cm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la conductivité et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
# superficie_champs_vs_pH

```{r superficie_champs_vs_pH}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,pH_units))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,pH_units))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=pH_units,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=pH_units),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=pH_units,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=pH_units),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le pH et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="pH_units") +
...
# superficie_champs_vs_Turbidité

```{r superficie_champs_vs_Turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# superficie_champs_vs_couleur

```{r superficie_champs_vs_couleur}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
# superficie_champs_vs_COT

```{r superficie_champs_vs_COT}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L),
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=log10(Carbon_Total_Organic_mg_L)),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L),

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=log10(Carbon_Total_Organic_mg_L)),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la COT et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# superficie_champs_vs_coliforme

```{r superficie_champs_vs_coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctF_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctF_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctF_supdrainee_1990, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctF_supdrainee_1990, y=E.coli_MPN_100.mL),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctF_supdrainee_2020, y=E.coli_MPN_100.mL,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctF_supdrainee_2020, y=E.coli_MPN_100.mL),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
```{r superficie_champs_vs_phosphore}
...
labs(title="Relation entre le phosphore et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
```{r superficie_champs_vs_nitrate}
...
labs(title="Relation entre le nitrate et les superficies des champs",
x=" Les superficies des champs (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Coupeablanc_vs_température

```{r Coupeablanc_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les Coupes à blanc",
...
# Coupeablanc_vs_Oxyg_dissous

```{r Coupeablanc_vs_Oxyg_dissous }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
# Coupeablanc_vs_Conductivité

```{r Coupeablanc_vs_Conductivité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,Conductivity_microS_cm))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,Conductivity_microS_cm))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=Conductivity_microS_cm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=Conductivity_microS_cm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=Conductivity_microS_cm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=Conductivity_microS_cm),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la Conductivité et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre la pH et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="pH_units") +
...
# Coupeablanc_vs_Turbidité_1

```{r Coupeablanc_vs_Turbidité_1 }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la Turbidité et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# Coupeablanc_vs_Turbidité_2

```{r Coupeablanc_vs_Turbidité_2 }
...
labs(title="Relation entre la couleur et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
labs(title="Relation entre la COT et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# Coupeablanc_vs_Coliforme

```{r Coupeablanc_vs_Coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=E.coli_MPN_100.mL),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=E.coli_MPN_100.mL,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=E.coli_MPN_100.mL),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le coliforme et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
# Coupeablanc_vs_phosphore

```{r Coupeablanc_vs_phosphore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctC_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctC_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctC_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctC_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctC_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctC_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
labs(title="Relation entre le nitrate et les coupes à blanc",
x=" Les coupes à blanc (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# Boisé_vs_température

```{r Boisé_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les superficies boisés",
...
subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
labs(title="Relation entre la conductivité et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre le pH et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="pH_units") +
...
```{r Boisé_vs_Turbidité}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la turbidité et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le COT et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
```{r Boisé_vs_coliforme}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=E.coli_MPN_100.mL),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=E.coli_MPN_100.mL,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=E.coli_MPN_100.mL),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
```{r Boisé_vs_phosphore}

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Phosphorus_Total_mg_L))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Phosphorus_Total_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Phosphorus_Total_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Phosphorus_Total_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Phosphorus_Total_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Phosphorus_Total_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le phosphore et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
subset(Period1, select=c(Station2, Date, pctB_supdrainee_1990,Nitrate_as_N_mg_L))
subset(Period2, select=c(Station2, Date, pctB_supdrainee_2020,Nitrate_as_N_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctB_supdrainee_1990, y=Nitrate_as_N_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctB_supdrainee_1990, y=Nitrate_as_N_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctB_supdrainee_2020, y=Nitrate_as_N_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctB_supdrainee_2020, y=Nitrate_as_N_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le nitrate et les superficies boisés",
x=" Les superficies boisés (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
# c: superficie de succession naturelle
...
# superficie_naturels_vs_température

```{r superficie_naturels_vs_température }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,Temp_eau_YSI_degC))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,Temp_eau_YSI_degC))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la température et les superficies naturels",
...
# superficie_naturels_vs_oxygène

```{r superficie_naturels_vs_oxygène }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,Oxyg_dissous_YSI_mg_L))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,Oxyg_dissous_YSI_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=Oxyg_dissous_YSI_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=Oxyg_dissous_YSI_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre l'oxygène et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Oxyg_dissous_YSI_mg_L") +
...
```{r superficie_naturels_vs_conductivité }
...
labs(title="Relation entre la conductivité et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Conductivity_microS_cm") +
...
labs(title="Relation entre le pH et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="pH_units") +
...
# superficie_naturels_vs_Turbidité

```{r superficie_naturels_vs_Turbidité }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,Turbidity_NTU))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,Turbidity_NTU))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=Turbidity_NTU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=Turbidity_NTU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=Turbidity_NTU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=Turbidity_NTU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le Turbidité et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Turbidity_NTU") +
...
# superficie_naturels_vs_couleur

```{r superficie_naturels_vs_couleur }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,Colour_TCU))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,Colour_TCU))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=Colour_TCU,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=Colour_TCU),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=Colour_TCU,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=Colour_TCU),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre la couleur et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Colour_TCU") +
...
# superficie_naturels_vs_COT

```{r superficie_naturels_vs_COT }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,Carbon_Total_Organic_mg_L))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,Carbon_Total_Organic_mg_L))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=Carbon_Total_Organic_mg_L,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=Carbon_Total_Organic_mg_L),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=Carbon_Total_Organic_mg_L,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=Carbon_Total_Organic_mg_L),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre le COT et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Carbon_Total_Organic_mg_L") +
...
# superficie_naturels_vs_coliforme

```{r superficie_naturels_vs_coliforme }

#names(Pok2001.2022) 

subset(Period1, select=c(Station2, Date, pctN_supdrainee_1990,E.coli_MPN_100.mL))
subset(Period2, select=c(Station2, Date, pctN_supdrainee_2020,E.coli_MPN_100.mL))

ggplot() +
geom_point(data=Period1, aes(x=pctN_supdrainee_1990, y=E.coli_MPN_100.mL,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=pctN_supdrainee_1990, y=E.coli_MPN_100.mL),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=pctN_supdrainee_2020, y=E.coli_MPN_100.mL,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=pctN_supdrainee_2020, y=E.coli_MPN_100.mL),
 method=lm, se=FALSE, color="blue") +
...
labs(title="Relation entre les coliformes et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="E.coli_MPN_100.mL") +
...
```{r superficie_naturels_vs_phosphore }
...
labs(title="Relation entre le phosphore et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Phosphorus_Total_mg_L") +
...
```{r superficie_naturels_vs_nitrate}
...
labs(title="Relation entre le nitrate et les superficies naturels",
x=" Les superficies naturels (fraction de la superficie du bassin)",
y="Nitrate_as_N_mg_L") +
...
Pok2001.2022$pctN_supdrainee_1990 = Pok2001.2022$precip.tot.2jours /
Pok2001.2022$precip.tot.21jours

Pok2001.2022$pctN_supdrainee_2020 = Pok2001.2022$precip.tot.6jours /
Pok2001.2022$precip.tot.21jours
...
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Temp_eau_YSI_degC)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre Température et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
       y="Temp_eau_YSI_degC")
...
p3 <- ggplot(Pok2001.2022, aes(x=precip.tot.14jours, y=Temp_eau_YSI_degC)) +
  ...
  labs(title="Relation entre Température et la Précipitation 14jrs avant",
...
```{r Oxygène_VS_Précip_total}
# Créer un graphique pour la précipitation à 2 jours plus tôt 
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Oxyg_dissous_YSI_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="red") +
  labs(title="Relation entre l'Oxygène et la Précipitation 2jrs avant",
       x="precip.tot.2jours",
       y="Oxyg_dissous_YSI_mg_L")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Oxyg_dissous_YSI_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre l'Oxygène et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
       y="Oxyg_dissous_YSI_mg_L")
# Créer un graphique pour la précipitation à 14 jours plus tôt 
p3 <- ggplot(Pok2001.2022, aes(x=precip.tot.14jours, y=Oxyg_dissous_YSI_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="yellow") +
  labs(title="Relation entre l'Oxygène et la Précipitation 14jrs avant",
       x="precip.tot.14jours",
       y="Oxyg_dissous_YSI_mg_L")
# Créer un graphique pour la précipitation à 21 jours plus tôt 
p4 <- ggplot(Pok2001.2022, aes(x=precip.tot.21jours, y=Oxyg_dissous_YSI_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="green") +
  labs(title="Relation entre l'Oxygène et la Précipitation 21jrs avant",
       x="precip.tot.21jours",
       y="Oxyg_dissous_YSI_mg_L")
# Afficher les graphiques
print(p1)
...
```{r conductivité_VS_Précip_total}
# Créer un graphique pour la précipitation à 2 jours plus tôt 
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Conductivity_microS_cm)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="red") +
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Conductivity_microS_cm")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Conductivity_microS_cm)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
       y="Conductivity_microS_cm")
# Créer un graphique pour la précipitation à 14 jours plus tôt 
p3 <- ggplot(Pok2001.2022, aes(x=precip.tot.14jours, y=Conductivity_microS_cm)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="yellow") +
  labs(title="Relation entre la conductivité et la Précipitation 14jrs avant",
       x="precip.tot.14jours",
       y="Conductivity_microS_cm")
# Créer un graphique pour la précipitation à 21 jours plus tôt 
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=pH_units)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="pH_units")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=pH_units)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Turbidity_NTU)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Turbidity_NTU")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Turbidity_NTU)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Colour_TCU)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Colour_TCU")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Colour_TCU)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Carbon_Total_Organic_mg_L)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Carbon_Total_Organic_mg_L")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Carbon_Total_Organic_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=E.coli_MPN_100.mL)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="E.coli_MPN_100.mL")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=E.coli_MPN_100.mL)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Phosphorus_Total_mg_L)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Phosphorus_Total_mg_L")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Phosphorus_Total_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
       y="Phosphorus_Total_mg_L")
# Créer un graphique pour la précipitation à 14 jours plus tôt 
p3 <- ggplot(Pok2001.2022, aes(x=precip.tot.14jours, y=Phosphorus_Total_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="yellow") +
  labs(title="Relation entre la conductivité et la Précipitation 14jrs avant",
       x="precip.tot.14jours",
       y="Phosphorus_Total_mg_L")
# Créer un graphique pour la précipitation à 21 jours plus tôt 
p4 <- ggplot(Pok2001.2022, aes(x=precip.tot.21jours, y=Phosphorus_Total_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="green") +
  labs(title="Relation entre la conductivité et la Précipitation 21jrs avant",
       x="precip.tot.21jours",
...
p1 <- ggplot(Pok2001.2022, aes(x=precip.tot.2jours, y=Nitrate_as_N_mg_L)) +
  ...
  labs(title="Relation entre la conductivité et la Précipitation 2jrs avant",
       ...
       y="Nitrate_as_N_mg_L")
# Créer un graphique pour la précipitation à 6 jours plus tôt 
p2 <- ggplot(Pok2001.2022, aes(x=precip.tot.6jours, y=Nitrate_as_N_mg_L)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE, color="blue") +
  labs(title="Relation entre la conductivité et la Précipitation 6jrs avant",
       x="precip.tot.6jours",
...
#code recu d'Alain le 16 juillet 2024 sur les arbres de régressions

```{r ctree1_2}
#NF_km2_supdrainee_1990

library("partykit") # pour ctree
library("strucchange") # for p values of ctree models

# Créer une variable "Mois"
Pok2001.2022$Mois<-format(as.Date(Pok2001.2022$Date, format="%Y-%m-%d"),"%m")
class(Pok2001.2022$Mois)
# Convertir la classe en facteur
Pok2001.2022$Mois=as.factor(Pok2001.2022$Mois)
class(Pok2001.2022$Mois)
# Voir les niveaux du facteur "Mois"
levels(Pok2001.2022$Mois)
table(Pok2001.2022$Mois)

# Créer une variable "Annee"
Pok2001.2022$Annee<-format(as.Date(Pok2001.2022$Date, format="%Y-%m-%d"),"%Y")
class(Pok2001.2022$Annee)
Pok2001.2022$Annee=as.factor(Pok2001.2022$Annee)
levels(Pok2001.2022$Annee)
table(Pok2001.2022$Annee)

# Présentation de la fonction "ctree" ("categorical tree") avec
# un modèle de température en fonction du mois de l'année
partykit::ctree(Temp_eau_YSI_degC~Mois, data=Pok2001.2022)
# Les températures du mois de juillet (moyenne=14,5 deg. C) se distinguent
# de celles des mois d'août et de septembre (12,6 deg. C) et de celles
# des mois d'octobre-novembre (6,3 deg. C)

# Ces divisions binaires sont-elles "statistiquement signficatives"?
ctree01_temp_mois=partykit::ctree(Temp_eau_YSI_degC~Mois, data=Pok2001.2022,
control=partykit::ctree_control(minsplit=1, minbucket=1,minprob=0.1,
maxdepth=1, testtype="Teststatistic"))
ctree01_temp_mois 
sctest(ctree01_temp_mois)
# À faire: comprendre pourquoi les valeurs p ne sont pas calculées
#plot(ctree01_temp_mois)

# Modèle avec plus de paramètres (à explorer)
# ctree01_temp_mois=partykit::ctree(Temp_eau_YSI_degC~Mois, data=Pok2001.2022,
#control=partykit::ctree_control(minsplit=1, minbucket=1,minprob=0.1,
#maxdepth=1,
...
# On peut ajouter plusieurs variables indépendantes "X" au modèle:
ctree02_temp=partykit::ctree(Temp_eau_YSI_degC~ Mois + Period, data=Pok2001.2022,
control=partykit::ctree_control(testtype="Teststatistic"))
ctree02_temp
# La première division se fait selon les mois: le mois de juillet se distinguent des mois
# d'août et septembre et ceux-ci se distinguent des mois d'octobre et novembre.
# Dans le mois de juillet, la période 1 a été plus chaude (16,2 deg. C en moyenne) 
# que la période 2 (13,6 deg. C).
sctest(ctree02_temp)
#plot(ctree02_temp)


# On peut ajouter plusieurs variables dépendantes "Y" au modèle:
ctree03_temp_pH=partykit::ctree(Temp_eau_YSI_degC+pH_units~Mois, data=Pok2001.2022,
control=partykit::ctree_control(testtype="Teststatistic"))
ctree03_temp_pH
sctest(ctree03_temp_pH)
#plot(ctree03_temp_pH)
 
# On peut ajouter plusieurs variables "Y" (tous les paramètres ensemble, 
# permettant ainsi de parler de "qualité de l'eau") et plusieurs variables X...

```

# essaie Gaël sur les arbres de régression
...
# Présentation de la fonction "ctree" ("categorical tree") avec
# un modèle de température en fonction du mois de l'année
partykit::ctree(Oxyg_dissous_YSI_mg_L~Mois, data=Pok2001.2022)
# Les températures du mois de juillet (moyenne=14,5 deg. C) se distinguent
# de celles des mois d'août et de septembre (12,6 deg. C) et de celles
# des mois d'octobre-novembre (6,3 deg. C)

# Ces divisions binaires sont-elles "statistiquement signficatives"?
ctree01_temp_mois=partykit::ctree(Oxyg_dissous_YSI_mg_L~Mois, data=Pok2001.2022,
control=partykit::ctree_control(minsplit=1, minbucket=1,minprob=0.1,
maxdepth=1))
ctree01_temp_mois
...
# Vérifier si le modèle contient des valeurs NaN
#any(is.nan(ctree01_temp_mois))
...
# On peut ajouter plusieurs variables indépendantes "X" au modèle:
ctree02_temp=partykit::ctree(Oxyg_dissous_YSI_mg_L~ Mois + Period, data=Pok2001.2022,
control=partykit::ctree_control(minsplit=1, minbucket=1,minprob=0.1,
maxdepth=1))
ctree02_temp
...
# On peut ajouter plusieurs variables dépendantes "Y" au modèle:
ctree03_temp_pH=partykit::ctree(Temp_eau_YSI_degC+Oxyg_dissous_YSI_mg_L+Conductivity_microS_cm+pH_units+Turbidity_NTU+Colour_TCU+Carbon_Total_Organic_mg_L+E.coli_MPN_100.mL+Phosphorus_Total_mg_L+Nitrate_as_N_mg_L~Mois, data=Pok2001.2022,
control=partykit::ctree_control(testtype="Teststatistic"))
ctree03_temp_pH
sctest(ctree03_temp_pH)
#plot(ctree03_temp_pH)
...
ctree03=partykit::ctree(Temp_eau_YSI_degC+Oxyg_dissous_YSI_mg_L+Conductivity_microS_cm+pH_units+Turbidity_NTU+Colour_TCU+Carbon_Total_Organic_mg_L+E.coli_MPN_100.mL+Phosphorus_Total_mg_L+Nitrate_as_N_mg_L~Annee+Mois, data=Pok2001.2022,
control=partykit::ctree_control(minsplit=1, minbucket=1,minprob=0.1,
maxdepth=1))
...
# Graphique pour expliquer la relation des précpitation sur les périodes de chaque année.

#precip_tot_mm

```{r, echo=F, message=F, warning=F, fig.cap = "précipitations totale mm jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
  precip.tot.1=ggplot(aes(y=precip_tot_mm, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.1
```

#precip.tot.2jours

```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.2jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.2=ggplot(aes(y=precip.tot.2jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.2
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.3jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.3=ggplot(aes(y=precip.tot.3jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.3
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.4jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.4=ggplot(aes(y=precip.tot.4jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.4
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.5jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.5=ggplot(aes(y=precip.tot.5jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.5
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.6jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.6=ggplot(aes(y=precip.tot.6jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.6
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.7jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.7=ggplot(aes(y=precip.tot.7jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.7
...
```{r, echo=F, message=F, warning=F, fig.cap = "precip.tot.14jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
precip.tot.14=ggplot(aes(y=precip.tot.14jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
precip.tot.14
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmax2jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmax2=ggplot(aes(y=Tmax2jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmax2
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmax3jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmax3=ggplot(aes(y=Tmax3jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmax3
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmax4jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmax4=ggplot(aes(y=Tmax4jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmax4
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmax5jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmax5=ggplot(aes(y=Tmax5jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmax5
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmax6jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmax6=ggplot(aes(y=Tmax6jours, x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmax6
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmin3jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmin3=ggplot(aes(y=Tmin3jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmin3
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmin4jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmin4=ggplot(aes(y=Tmin4jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmin4
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmin5jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmin5=ggplot(aes(y=Tmin5jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmin5
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tmin6jours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmin6=ggplot(aes(y=Tmin6jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmin6
...
```{r, echo=F, message=F, warning=F, fig.cap = "Tminjours"}
# AP 2024-06-13 Essayer l'option "fig.cap" ci-dessus, ici et partout.
# Température variabilité spatiale
tmin7=ggplot(aes(y=Tmin7jours , x= Period), data=Pok2001.2022) + geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust=1),axis.text=element_text(size=12),
        axis.title=element_text(size=14,face="bold"))+
  ggtitle("Variation  de la pluies à 20ans entre les deux Period")
tmin7
...
La base des données météorologiques est constituée des données de la météo collectés sur la station de Bas-Caraquet et de Barthus. Selon les données de la précipitation, la période 1 à connu moins de pluie par rapport à la période 2 quand ont prends en compte la variabilité de précipitation de 4 jours à 21 jours avant les prélèvements. La précipitation au jours du prélèvement jusqu’au troisièmes jours avant le prélèvement ont connu plus de pluie dans la période 2 par rapport à la période 1. La période 1 malgré les plus faibles précipitations par rapport à la période 2 à connu l’augmentation des températures maximum de 1 à 7 jours avant les prélèvements. La température de l’air (à Bas-Caraquet) durant la période 1 était généralement plus élevé par rapport à la période 2. Elle a moyennement augmenté de 3% du 1 jusqu’au 7ème jours avant le prélèvement.
...
t.test(Pok2001.2022$precip_tot_mm~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.2jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.3jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.4jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.5jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.6jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.7jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.14jours~Pok2001.2022$Period)
t.test(Pok2001.2022$precip.tot.21jours~Pok2001.2022$Period)

```

# Les temptempératures

```{r, echo=F, message=F, warning=F }

t.test(Pok2001.2022$Tmax2jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmax3jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmax4jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmax5jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmax6jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmax7jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin2jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin3jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin4jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin5jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin6jours~Pok2001.2022$Period)
t.test(Pok2001.2022$Tmin7jours~Pok2001.2022$Period)

```

#Ne compte pas

# Somme_ordre_vs_Conductivité_1

```{r Somme_ordre_vs_Conductivité_1}
...
#Correlation entre Conductivité et pluies

```{r Zone_industriel_vs_Conductivité_1}
...
ggplot() +
geom_point(data=Period1, aes(x=Conductivity_microS_cm, y=precip_tot_mm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=Conductivity_microS_cm, y=precip_tot_mm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=Conductivity_microS_cm, y=precip_tot_mm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=Conductivity_microS_cm, y=precip_tot_mm),
 method=lm, se=FALSE, color="blue") +
...
```{r Zone_industriel_vs_Conductivité_2}
...
ggplot() +
geom_point(data=Period1, aes(x=Conductivity_microS_cm, y=Temp_eau_YSI_degC,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=Conductivity_microS_cm, y=Temp_eau_YSI_degC),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=Conductivity_microS_cm, y=Temp_eau_YSI_degC,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=Conductivity_microS_cm, y=Temp_eau_YSI_degC),
 method=lm, se=FALSE, color="blue") +
...
```{r Zone_industriel_vs_Conductivité_3}
...
ggplot() +
geom_point(data=Period1, aes(x=Carbon_Total_Organic_mg_L, y=precip_tot_mm,
 color="P1", shape="P1")) +
geom_smooth(data=Period1, aes(x=Carbon_Total_Organic_mg_L, y=precip_tot_mm),
            
 method=lm, se=FALSE, color="red") +
 geom_point(data=Period2, aes(x=Carbon_Total_Organic_mg_L, y=precip_tot_mm,

color="P2", shape="P2")) +

 geom_smooth(data=Period2, aes(x=Carbon_Total_Organic_mg_L, y=precip_tot_mm),
 method=lm, se=FALSE, color="blue") +

...
plot(meteo$precip.tot.21jours ~ meteo$Date, type="b", pch="c", col=1, lty=1)
points(meteo$precip.tot.14jours ~ meteo$Date, type="b", pch="b", col=2, lty=2)
points(meteo$precip.tot.7jours ~ meteo$Date, type="b", pch="a", col=3, lty=3)
```
...
plot(y1$Temp_eau_YSI_degC, y1$Conductivity_microS_cm)
text(y1$Temp_eau_YSI_degC, y1$Conductivity_microS_cm, y1$Station)
...
autoplot(pigsPCA1Richibucto, label=T, data=pigs0Richibucto, size=3, colour="Stream", shape='Stream', loadings=T, loadings.label=T, loadings.label.size=4) + theme_bw() + theme(legend.position = "bottom")+ ggtitle("labels indicate Stream, median depth (cm)")
